...
def learnPredictor(trainExamples: List[Tuple[Any, int]], validationExamples: List[Tuple[Any, int]],
    ...
    for i in range(numEpochs):
        for x, y in trainExamples:
            feature_v = featureExtractor(x)
            k = dotProduct(feature_v, weights)
            h = 1/(1+math.exp(-k))

            scale = -1 * alpha * (h - y) if y != -1 else (-1 * alpha * h)
            increment(weights, scale, feature_v)
...
def generateDataset(numExamples: int, weights: WeightVector) -> List[Example]:
    ...
    def generateExample() -> Tuple[Dict[str, int], int]:
        ...
        sample_weight = random.sample(list(weights.keys()), random.randint(1, len(weights)))
        phi = {key: 1 for key in sample_weight}
...
def extractCharacterFeatures(n: int) -> Callable[[str], FeatureVector]:
    ...
    def extract(x: str) -> Dict[str, int]:
        ...
        d = defaultdict(int)
        for i in range(len(new_str)-n+1):
            d[new_str[i:i+n]] += 1
        return d
        # END_YOUR_CODE
